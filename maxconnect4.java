...
*/
    ...
    //
    ...
    else if(game_mode.equals("interactive")){
    	/////////////   interactive mode ///////////
    	
    	String next = args[2].toString();	// the next player
    	if (next.equals("human-next")){
    		if (currentGame.getCurrentTurn() == 1){
    			calculon.sethuman(1);
    			calculon.setcomputer(2);
    		}
    		if (currentGame.getCurrentTurn() == 2){
    			calculon.sethuman(2);
    			calculon.setcomputer(1);
    		}
    		
    	}
    	else if(next.equals("computer-next")){
    		if (currentGame.getCurrentTurn() == 1){
    			calculon.sethuman(2);
    			calculon.setcomputer(1);
    		}
	    ...
	    while( currentGame.getPieceCount() < 42 ) 
	    {
	        ...
	        if (next.equals("human-next")){
	            ...
	            while(true){
		            ...
		            if(currentGame.isValidPlay(move)){
		            	currentGame.playPiece(move);
		            	currentGame.printGameBoardToFile("human.txt");
		            	break;
		            }
		            else{
		            	System.out.println("COLUMN INPUT NOT VALID. TRY AGAIN ");
		            }
	            }
	            // display the current game board
		        System.out.println("move " + currentGame.getPieceCount() 
    	...
    	*/
    	...
    	System.out.println("Game mode is NOT one_move or interactive");
    	exit_function( 0 );
    }
    return;
